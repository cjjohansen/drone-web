...
* By staying only in the Refine phase, we will be required to produce an OpenAPI Specification of each API, an AsyncAPI Specification for all emitted events/commands identified in the Define phase, and a README document that demonstrates HTTP request/response examples for each job story
* The AsyncAPI specs will document all domain events emitted by API operations, their payloads, and channels — this will later feed into Event Catalog for event discovery and governance
* We will not consider the Refine phase complete until we have completed all artifacts for this phase (OpenAPI specs, AsyncAPI specs, and README examples)
...
Next, generate AsyncAPI specs for the events identified in the Define phase. These will later be imported into Event Catalog for event discovery, governance, and cross-team collaboration.

```
Generate a fully compliant AsyncAPI 3.0 specification that documents all domain events emitted by the API operations identified in the Define phase. For each event, include the channel name, event payload schema with required fields and examples, and metadata (source boundary, triggering operation). Use a consistent naming convention of {BoundaryName}.{ResourceName}.{EventName} for channel names. Group events by their source boundary. Examples should be consistent with the OpenAPI spec examples to demonstrate end-to-end workflows. Start with the first API boundary.
```
